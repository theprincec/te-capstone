...
If the script fails with a permission to execute error, run the command to `chmod -x create.sh` to add the execute permission to the script and then try to run the script again.
...
A Datasource has been configured for you in `/src/resources/application.properties`. You can change the name of this database if you want, but remember to change it here and in the `create.sh` script in the database folder:
